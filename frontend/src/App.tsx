...
function ResearchResultsSkeleton() {
  return (
    ...
    >
      <h2>Results</h2>
      <p className="rg-skeleton-hint">Identifying jurisdiction…</p>
...
function App() {
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    void executeResearch(undefined);
  };
  ...
  return (
    <div className="rg-app">
      ...
      <form onSubmit={submit} className="rg-panel rg-no-print" autoComplete="off">
        ...
        <div className="rg-row rg-grid-2">
          <div>
            ...
            {!useMapsAddr ? (
              ...
              >
                Type a U.S. street address and pick a suggestion—Universal Scout starts as soon as an
                address + ZIP resolve. Or use{' '}
                <em>Run research</em> afterwards. The server runs{' '}
                <code className="rg-code-hint">geocode.py</code> on the formatted line for city vs county
                permit routing; ZIP is cross-checked with your selection.
              </p>
            )}
          </div>
          <div>
        ...
        {loading ? (
          <p className="rg-await-jurisdiction rg-no-print" role="status" aria-live="polite">
            <span className="rg-spinner" aria-hidden />
            Identifying jurisdiction… (Universal Scout)
          </p>
        ) : null}
        <button
